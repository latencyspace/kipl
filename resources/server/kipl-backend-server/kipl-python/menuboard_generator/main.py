...
key_path = glob.glob("jnu-team-03-2-dataflow-credential.json")[0]
# Google Cloud의 서비스 계정 키 파일 경로
KEY_PATH = "jnu-team-03-2-dataflow-credential.json"
# Credentials 객체 생성
...
def run_pipeline():
    ...
    options.view_as(beam.options.pipeline_options.GoogleCloudOptions).project = 'jnu-team-03-2'
    options.view_as(beam.options.pipeline_options.GoogleCloudOptions).region = 'us'
    options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).staging_location = 'gs://temp_bucket_01110/staging_location'
    options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).temp_location = 'gs://temp_bucket_01110/temp_location'
    ...
    with (beam.Pipeline(options=options) as p):
        ...
        uuid = b'0c539a9ce4944c98b50bc2690eaf7acd'
        ...
        changed_values = (
                p
                | "Create input collection" >> beam.Create(input_data)
                | "Change values" >> beam.ParDo(Allergy_Value_DoFn(input_data, uuid.decode('utf-8')))
        )
        changed_values2 = (
                changed_values
                | "value to dict" >> beam.Map(change_value_to_dict)
        )

        changed_values3 = (
                changed_values
                | "dict to num" >> beam.Map(change_value_to_num)
